...
def hoi(potentials, data_cleaned):
    ...
    for pair in potentials:
        ...
        if (data_cleaned['shop'][i] == data_cleaned['shop'][j]) or (combi_brands[i] != combi_brands[j]):
            del_keys.append(pair)

    print("potential pairs LSH removed due to same shop or different brand =", len(del_keys))
    return del_keys
...
def evaluateResults(candidatepairs, candidate_pairsCluster, data_cleaned, t):
    ...
    print("true positives found Clustering =  ", tpLSH)
    ...
    # numerator = (numberCandidatesLSH - len(clean_lsh))
...
bootstraps = 1
print('Bootstrap number ' + str(bootstraps) )
for i in range(bootstraps):
    print("|____________________________________________ Bootstrap " + str(
        i + 1) + " ____________________________________________|")
    index_hold = []
    for j in range(len(df)):
        integer = random.randint(1, len(df))
        if integer not in index_hold:
            index_hold.append(integer)
    indices_delete = []
    #for k in range(len(df)):
     #   if k not in index_hold:
